...
class DesignStandardsChecker {
    constructor() {
        ...
        this.config = this.loadConfigFromDocument() || Object.assign({}, this.defaultConfig);
    }
    ...
    importConfigFromTeamLibrary(fileKey) {
    ...
    runAllChecks(node = figma.currentPage, isDeepCheck = true) {
        const results = {
            passed: true,
            issues: [],
            stats: {
    ...
    processNode(node, results, isDeepCheck) {
    ...
    checkTypography(node, results) {
        if (node.type === 'TEXT') {
            ...
            if (!node.textStyleId) {
                ...
                figma.getStyleByIdAsync(node.textStyleId).then(textStyle => {
                    if (textStyle) {
                        const isValidStyle = this.config.typography.textStyles.some(pattern => textStyle.name.includes(pattern));
                        if (!isValidStyle) {
    ...
    checkAccessibility(node, results) {
        // Check text contrast
        ...
        if (this.isInteractiveElement(node) && 'width' in node && 'height' in node) {
            ...
            if (width < this.config.accessibility.interactiveElementMinSize ||
                ...
                this.addIssue(results, "accessibility", `Interactive element "${node.name}" (${width}x${height}) is too small for touch targets`, node.id);
            }
        }
    }
    // Helper functions
    ...
    hasComponentNamed(node, componentName) {
        if (!('children' in node))
            return false;
        for (const child of node.children) {
            if (child.name.includes(componentName))
                return true;
            // Recursive check
            if ('children' in child) {
                if (this.hasComponentNamed(child, componentName))
                    return true;
            }
        }
        return false;
    ...
    hasAdequateContrast(_color1, _color2) {
    ...
    addIssue(results, category, message, nodeId) {
        results.issues.push({
            category,
            message,
            nodeId
            ...
            case 'focus-node':
                ...
                figma.getNodeByIdAsync(msg.nodeId).then(focusNode => {
                    if (focusNode && 'type' in focusNode && focusNode.type !== 'PAGE' && focusNode.type !== 'DOCUMENT') {
                        figma.currentPage.selection = [focusNode];
